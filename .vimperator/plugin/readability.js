...
function getCurrentTab() {
  var recent = Components.classes["@mozilla.org/appshell/window-mediator;1"].
    getService(Components.interfaces.nsIWindowMediator).
    getMostRecentWindow("navigator:browser");
  return recent ? recent.content : null;
}
...
function saxParser(elem, callbacks){
  ...
  function parse(node){
    ...
    for(var i = 0; i < num; i++){
      ...
      if(nodeType === 3 /*text*/)
        callbacks.ontext(childs[i].textContent.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;"));
...
commands.addUserCommand(
    "read[er]",
    "fall in reading mode",
    function () {
      var window = getCurrentTab(),
        document = window.document,
        readable = new Readability();
      readable.setSkipLevel(3);
      saxParser(document.childNodes[document.childNodes.length-1], readable);
      var art = readable.getArticle();
      window.console.log(art.html);
      document.head.innerHTML = "<title>" + art.title + "</title>";
      document.body.innerHTML = "<h1>" + art.title + "</h1>" + art.html;
